...
function loadMap14_2(){
    ...
    setPos(box1,2*w+.5*d,0.5*w);

    setPos(box2,0.5*w,1.5*d);
    setPos(box1,1*w+0.5*d,1*d+0.5*w);
    setPos(box2,0.5*w+w+d,0.5*d+w);

    setPos(box1,0.5*d,0.5*w+2*d);
    setPos(box2,0.5*w+d,0.5*d+w+d);
    setPos(box2,1.5*w+d,0.5*d+w+d);
