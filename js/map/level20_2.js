...
function loadMap20_2(){
    ...
    for(var i=0;i<4;i++){
        ...
        mesh.position.z = -50+0.5*d;
        mesh.position.x = -60+2*d+w*i+0.5*w;
        mesh.position.y = gd;
        scene.add( mesh );
        mesh.state=state;
        mapMeshs.push(mesh);
    ...
    for(var i=0;i<7;i++){
        ...
        mesh.position.x = -60+w+d*i+0.5*d;
        mesh.position.y = gd;
        scene.add( mesh );
        mesh.state=state;
        mapMeshs.push(mesh);
    ...
    for(var i=0;i<4;i++){
        var mesh = new THREE.Mesh( box2.clone(), new THREE.MeshBasicMaterial({color:Math.random()*0xffffff,transparent:true,opacity:1,wireframe:true}) );
        mesh.position.z = -50+2*w+0.5*d+i*d;
